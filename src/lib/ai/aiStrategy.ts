 ...
 *
 * ロードマップ (docs/ml/ML_IMPLEMENTATION_ROADMAP.md Phase 4.2) 本来の値は 0.6。
 * 一時的に 0.2 まで下げていたが、これは「52 クラス分類 + 推論時に再正規化
 * しない」という旧実装の挙動に合わせた値だった。旧実装は 52 枚分の確率から
 * 手札分だけを抜き出して正規化せずに返していたため、出せないカードに確率
 * 質量の大半が残り、confidence が構造的に 0.10〜0.25 に張り付いていた。
 *
 * 2026-08-01 に推論側を候補スコアリング (合法手だけをスコアして、その中で
 * 正規化する) へ移行し、confidence が「合法手の中でこの手が選ばれる確率」
 * になった。テストセット (34,840 行 / 903 ゲーム) で中央値 0.588、
 * confidence の帯と的中率がほぼ一致する。よってロードマップ本来の 0.6 に戻す。
 *
 * 0.6 での実効値 (合法手が 1 枚しかない局面は下の selectAICard が ML を呼ばずに
 * 短絡するので、それらを除いた「非強制手」ベース):
 *   採用率 36.45% / 採用時に参照ポリシーと 81.13% 一致。
 *
 * ⚠️ この値と Hugging Face Space のモデルはセットで動く。Space が旧 52 クラス
 * モデルのままだと /api/predict-card が 503 を返すので ML は発火しない (安全側)。
 * 逆に 0.2 のまま新モデルへ再学習すると、非強制手のほぼ全て (採用率 98.2%) で
 * ML が採用され、MCTS が事実上無効化される。順序と根拠は
 * docs/ml/CARD_PREDICTION_MODEL.md「再学習と閾値変更の順序依存」を参照。
 */
